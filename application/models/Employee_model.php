...
class Employee_model extends CI_Model {
	...
	{
		$this->db->select('A.ed_id,A.ed_emp_id,A.ed_emp_name,A.ed_emp_pass,A.ed_emp_email,A.ed_emp_desig,A.ed_emp_grade,A.ed_emp_div,A.ed_emp_team,A.ed_emp_section,A.ed_emp_leader,A.ed_emp_dob,A.ed_emp_doj,A.ed_emp_add_on,A.ed_emp_add_by,A.ed_emp_name');
    ...
    function saveBulkEmployee()
    {
	    ...
	    for($i=2;$i<=$totalrows;$i++)
	    {
	        ...
	        $data_emp['ed_emp_desig']		= $objWorksheet->getCellByColumnAndRow(3,$i)->getCalculatedValue();
	        $data_emp['ed_emp_grade']		= $objWorksheet->getCellByColumnAndRow(4,$i)->getCalculatedValue();
	        ...
	        if($data_emp['ed_emp_id']!='' && $data_emp['ed_emp_name']!='' && $data_emp['ed_emp_pass']!=''){
	        	...
	        	if($action_type=='insert'){
	        		...
	        		$this->db->select('c.ed_id');
					$this->db->from('be_emp_db as c');
					$this->db->where('c.ed_emp_id',strval($data_emp['ed_emp_id']));
					$query = $this->db->get();
					...
					if($query -> num_rows() == 1) { 
				 		...
				 		$this->db->where('ed_emp_id',strval($data_emp['ed_emp_id']));
